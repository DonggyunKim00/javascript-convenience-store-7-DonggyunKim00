...
- [x] 상품을 구매할 수 있다.
  - [x] 구매한 개수만큼 해당 상품의 재고 개수를 줄일 수 있다.
...
- [x] 쇼핑 리스트와 멤버쉽 유무를 받아 영수증을 생성할 수 있다.
- [x] 구매 상품 내역을 조회할 수 있다.
  - [x] 구매한 상품명, 수량, 가격 형태의 리스트로 반환한다.
- [x] 증정 상품 내역을 조회할 수 있다.
  - [x] 증정 상품명, 수량 형태의 리스트로 반환한다.
- [x] 금액 정보를 조회할 수 있다. (아래의 내용을 객체 형태로 반환한다.)
  - [x] 총구매액(구매한 상품의 총 수량과 총 금액)
  - [x] 행사할인(프로모션에 의해 할인된 금액)
  - [x] 멤버십할인(멤버십에 의해 추가로 할인된 금액)
    - [x] 멤버십 회원은 프로모션 미적용 금액의 30%를 할인받을 수 있다. (프로모션 상품을 제외한 금액의 30%)
    - [x] 프로모션 적용 후 남은 금액에 대해 멤버십 할인을 적용할 수 있다.
...
- [x] 살아있는 기능명세서를 만들었는가?
- [x] 관련 함수를 묶어 클래스를 만들고, 객체들이 협력하여 하나의 큰 기능을 수행하도록 하였는가?
- [x] 클래스와 함수에 대한 단위 테스트를 통해 의도한 대로 정확하게 작동하는 영역을 확보하였는가?
- [x] 3주차 공통 피드백을 적용하였는가?
  - [x] 객체를 객체답게 사용하였는가? => getter를 사용하는 대신 객체에 메시지를 보냈는가?
  - [x] 테스트 코드를 리팩터링 하였는가?
- [x] 질문에 대한 최종 회고를 진행하였는가?
- [▲] TDD 방식을 사용하였는가?
...
- [x] 입출력을 담당하는 클래스를 별도로 구현하였는가? (`InputView`, `OutputView`)
- [x] `@woowacourse/mission-utils`에서 제공하는 `Console` 및 `DateTimes` API를 사용하여 구현하였는가?
